...
typedef struct node_def node; //alias for the node struct
typedef struct root_def root; //alias for the root struct
typedef struct internalNode_def internal; //alias for internal node struct

//variables
const int size = 12; //the size of the workload
const int fanout = (size/3) + 3;  //the fanout enuring only a depth of three 
const int loop = (size/3);  //defines how the input workload will be divided
const internal *level2[loop];  //stores pointers for internal nodes after bulk load

/*
A struct that contructs a node
Variables; int key, int* ptr
*/
struct node_def
{
    int key;
...
/*
A struct that contructs a root node in the form of an array
Variables; node array[fanout]
*/
struct root_def
{
    node array[fanout];
...
/*
A struct that contructs a internal node in the form of an array
Variables; node array[fanout]
*/
struct internalNode_def
{
    node array[fanout];
};
...
void bulkload(int arr[size]);
void insertInternalNode();
void split(int* internal_node);
void insert();
